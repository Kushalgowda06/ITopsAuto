...
const MainLayout = () => {
  ...
  const fetchProblemTasks = async () => {
    try {
      const response = await serviceNowAxios.get(`/api/now/table/problem_task?sysparm_query=stateNOT IN6,7^numberINPTASK0010326,PTASK0010327,PTASK0010329,PTASK0010330&sysparm_fields=sys_id,number,problem.number,caller_id,location,sys_created_by,opened_by,sys_created_on,state,hold_reason,cmdb_ci,priority,category,subcategory,assignment_group,assigned_to,short_description,description,comments_and_work_notes`);
