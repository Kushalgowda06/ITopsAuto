...
const KnowledgeAssist = ({ categories = [], onCategoryClick }) => {
  ...
  const [mergedSOPs, setMergedSOPs] = useState([
    {
      id: 1001,
      title: 'Merged SOP: Windows Administration - Password Management',
      content: '=== Source 1: Windows Password Reset via Command Line ===\nAuthor: John Smith | Created: 2024-01-10 | Views: 245 | Rating: 4.2\n\nStep 1: Boot from recovery disk\nStep 2: Navigate to command prompt\nStep 3: Use net user command to reset password\n\n=== Source 2: Advanced Windows Password Recovery ===\nAuthor: Mike Johnson | Created: 2024-01-12 | Views: 189 | Rating: 4.0\n\nMethod 1: Using Windows PE disk\nMethod 2: Registry editing approach\nMethod 3: Local Security Policy modifications',
    ...
    {
      id: 1002,
      title: 'Merged SOP: Network Management - Connectivity Solutions',
      content: '=== Source 1: Basic Network Troubleshooting ===\nAuthor: Sarah Wilson | Created: 2024-01-08 | Views: 312 | Rating: 4.5\n\nStep 1: Check physical connections\nStep 2: Verify IP configuration\nStep 3: Test with ping commands\n\n=== Source 2: Advanced Network Diagnostics ===\nAuthor: Lisa Chen | Created: 2024-01-15 | Views: 278 | Rating: 4.3\n\n1. Network adapter analysis\n2. DNS resolution testing\n3. Router configuration review',
      ...
      rating: '4.4',
      mergedFrom: 2,
      mergedDate: '2024-01-21'
    },
    {
      id: 1003,
      title: 'Merged SOP: Database Management - Backup Strategies',
      content: '=== Source 1: SQL Server Backup Best Practices ===\nAuthor: Alex Wilson | Created: 2024-01-05 | Views: 156 | Rating: 4.8\n\nStep 1: Schedule regular full backups\nStep 2: Implement differential backups\nStep 3: Configure transaction log backups\n\n=== Source 2: Enterprise Database Backup Solutions ===\nAuthor: Emily Rodriguez | Created: 2024-01-18 | Views: 134 | Rating: 4.6\n\n1. Automated backup scheduling\n2. Offsite storage configuration\n3. Recovery testing procedures',
  ...
  const callMergeKBArticlesAPI = async (kbArticle1, kbArticle2) => {
    try {
      setIsLoading(true);
      const response = await fetch('http://172.31.6.97:6800/llm_generated/api/v1/merge_kb_articles/', {
        method: 'POST',
        headers: {
  ...
  const callStoreKBArticlesAPI = async (kbArticle) => {
    try {
      setIsLoading(true);
      const response = await fetch('http://172.31.6.97:6800/llm_generated/api/v1/store_kb_articles/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const callRefineKBArticlesAPI = async (kbArticle) => {
    try {
      setIsLoading(true);
      const response = await fetch('http://172.31.6.97:6800/llm_generated/api/v1/refine_kb_articles/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveToKnowledgeBase = async () => {
    try {
      ...
      setConfirmationStatus(500);
      setConfirmationMessage('Error: Failed to connect to storage service');
      setShowConfirmationModal(true);
    }
  };
  ...
  const getFilteredAndSortedData = () => {
    let filteredData = [...commonIssues, ...commonIssues, ...commonIssues, ...commonIssues];
  ...
  const kpiData = [
    ...
    },
    {
      ...
      value: '5', // Will be updated after lowScoringArticles is defined
      icon: HiChartBarSquare,
      color: '#feca57',
      description: 'Articles rated below 3 stars'
    },
    {
      title: 'Avg Duration to Solve',
      value: '2.4h',
      icon: HiClock,
      color: '#a8e6cf',
  ...
  const commonIssues = [
    {
      incidents: ['INC234124', 'INC234125'],
      additionalCount: 38,
      description: 'Windows 10 Login Issues',
      priority: 'High',
      category: 'Windows Administration',
      status: 'In Progress',
    ...
    {
      incidents: ['INC235001', 'INC235002'],
      additionalCount: 25,
      description: 'Office 365 Email Sync Problems',
      priority: 'Medium',
...
LangGraph helps in building agentic systems where agents can interact in a graph-based workflows. It supports both sync and async executions, but sometimes async flows are hard to debug. Agents can be defined with roles and memory, but memory management is not always stable. LangGraph makes it easy to create modular agents, but some users find it too complex for simple tasks. It also allows chaining agents, but chaining too many agents may slow down the system. Overall, LangGraph is a good tool for building smart agentic apps, but requiers careful design and testing.`,
  ...
  const lowScoringArticles = [
    {
      id: 1,
      title: 'Databricks Processing Guide',
      content: `1)Databricks Scope 
...
5)Fine-tune on domain-specific data`,
    ...
    {
      id: 3,
      title: 'Agent Framework Implementation',
      content: `1)Define Agent Roles 
...
Following these steps should help you diagnose and resolve the issue of the eth0 interface being down. If the problem persists after performing all these checks and actions, consider seeking further assistance from your network administrator or contacting technical support.`,
  ...
  kpiData[4].value = filteredMergedSOPs.length.toString(); // Merged Articles
  ...
  return (
    <Container fluid className="theme-fade-in" style={{ padding: '1rem 1.5rem' }}>
      ...
      {/* KPI Cards */}
      <Row className="theme-mb-lg justify-content-between">
      ...
      <div className="theme-card theme-mb-lg">
        <div className="theme-card-header" style={{ padding: '1rem 1.5rem 0.5rem' }}>
          <h5 className="theme-card-title" style={{ fontSize: '1.2rem', marginBottom: '0' }}>
            ...
            Knowledge Base Categories Performance
          </h5>
          <p className="text-muted mt-1" style={{ fontSize: '0.9rem', marginBottom: '0' }}>
            Real-time analytics across all documentation categories
          </p>
        </div>
        <div className="theme-card-body" style={{ padding: '1rem 1.5rem 1.5rem' }}>
          <div className="categories-carousel-container" style={{ 
            height: '120px', 
            overflow: 'hidden',
            position: 'relative',
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div className="stats-row" style={{ 
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                          <span style={{ fontSize: '0.65rem', color: '#28a745', fontWeight: '600' }}>
                            ✓{stats.success}
                          </span>
                          <span style={{ fontSize: '0.65rem', color: '#dc3545', fontWeight: '600' }}>
                            ✗{stats.failed}
                          </span>
                        </div>
                        <div style={{ 
          ...
          <div className="theme-card" style={{ height: '600px', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="theme-card-body" style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #e9ecef', flexShrink: 0 }}>
              <Row className="g-2 align-items-end">
                {/* Search Bar */}
                <Col md={4}>
                  <Form.Label style={{ fontSize: '0.8rem', marginBottom: '0.25rem', fontWeight: '600' }}>
                ...
                <Col md={3}>
                  ...
                  <div className="d-flex gap-1">
                    ...
                    <Button
                      ...
                      style={{ minWidth: '35px', fontSize: '0.7rem' }}
                ...
                <Col md={1} className="text-end">
                  <div style={{ fontSize: '0.8rem', marginBottom: '0.25rem', fontWeight: '600', opacity: 0 }}>
                    Actions
                  </div>
                  <Button
            ...
            <div className="theme-card-body" style={{ 
              ...
              <Table hover className="mb-0 table-fixed" style={{ minWidth: '1400px', tableLayout: 'fixed' }}>
                <thead style={{ background: '#f8f9fa', position: 'sticky', top: 0, zIndex: 10 }}>
                  <tr>
                    <th style={{ padding: '0.6rem 0.75rem', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap', minWidth: '120px' }}>
                    ...
                    <th style={{ padding: '0.6rem 0.75rem', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap', minWidth: '120px' }}>Category</th>
                    <th 
                      style={{ 
                        padding: '0.6rem 0.75rem', 
                        fontWeight: '600', 
                    ...
                    <th 
                      ...
                      onClick={() => handleSort('affectedUsers')}
                      title="Click to sort by affected users"
                    >
                      Affected Users
                      {sortField === 'affectedUsers' && (
                        <span style={{ marginLeft: '0.3rem', fontSize: '0.8rem' }}>
                          {sortDirection === 'asc' ? '↑' : '↓'}
                        </span>
                      )}
                    </th>
                    <th style={{ padding: '0.6rem 0.75rem', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap', minWidth: '110px' }}>
                      <HiClock style={{ marginRight: '0.3rem', fontSize: '0.9rem' }} />
                      SLA Status
                    </th>
                    <th style={{ padding: '0.6rem 0.75rem', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap', minWidth: '110px' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    getFilteredAndSortedData().map((issue, index) => {
                      ...
                      <tr key={index} style={{ borderBottom: '1px solid #dee2e6' }}>
                        ...
                        <td style={{ 
                          ...
                          >
                            <HiCalendarDays style={{ fontSize: '0.8rem', color: '#6c757d' }} />
                        ...
                        <td style={{ 
                          ...
                          <span 
                            style={{ color: '#dc3545' }}
                            title={`${issue.affectedUsers} users affected`}
                          >
                            {issue.affectedUsers}
                          </span>
                        </td>
                        <td style={{ 
                          ...
                          <span
                            className="badge"
                            style={{
                              background: getSLAStatusColor(issue.slaStatus),
                              color: 'white',
                              fontSize: '0.7rem',
                              padding: '0.25rem 0.5rem',
                              borderRadius: '10px'
                            }}
                            title={`SLA Status: ${issue.slaStatus}`}
                          >
                            {issue.slaStatus}
                          </span>
                        </td>
                        <td style={{ 
                          ...
                          <Button
                            ...
                            onClick={() => handleCreateDoc(issue.description)}
                            title={`Create documentation for: ${issue.description}`}
                          >
                            <HiDocumentText style={{ marginRight: '0.25rem', fontSize: '0.8rem' }} />
                            Create Doc
                          </Button>
                        </td>
                        </tr>
                ...
                {[
                  ...
                  <div 
                    ...
                    style={{
                      padding: '0.5rem',
                      marginBottom: '0.5rem',
                      background: index === 0 ? 'linear-gradient(135deg, #28a74520, #28a74510)' : '#f8f9fa',
                      borderRadius: '6px',
                      border: index === 0 ? '1px solid #28a74530' : '1px solid #e9ecef',
                      transition: 'all 0.3s ease',
                      cursor: 'pointer'
                    }}
                    onMouseEnter={(e) => {
                  ...
                  >
                    <div style={{ marginBottom: '0.3rem' }}>
                      ...
                      <div className="d-flex justify-content-between align-items-center">
                        ...
                        <span style={{ 
                          fontSize: '0.7rem',
                          color: article.trend.includes('+') ? '#28a745' : '#dc3545',
                          fontWeight: '600'
                        }}>
                          {article.trend}
                        </span>
      ...
      <Modal 
        ...
        keyboard={!isMerging}
      >
        <Modal.Header closeButton={!isMerging} style={{ background: 'var(--bg-gradient)', color: 'white' }}>
          <Modal.Title>
          ...
          {isMerging && (
            <div className="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" 
              ...
              <div className="text-center">
                ...
                <p className="text-muted">Analyzing and merging duplicate articles...</p>
          ...
          {isMerging && (
            <div className="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" 
              ...
              <div className="text-center">
                ...
                <p className="text-muted">Please wait while we process and merge the duplicate articles...</p>
              </div>
            </div>
          )}
          {selectedSOP && (
            <div>
              <div className="mb-4">
                <h4 style={{ color: '#333', fontWeight: '600' }}>
                  {selectedSOP.title}
                </h4>
              ...
              <div className="mb-4 p-3" style={{ 
                ...
                <Row>
                  ...
                  <Col md={6}>
                    ...
                    <div className="mb-2">
                      <strong className="text-muted">Sources Merged:</strong>
              ...
              <div className="mb-4">
                ...
                {!isEditingMergedSOP ? (
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: mergedSOP.htmlContent || mergedSOP.content }}
                  />
                ) : (
                  <div>
...
Regular paragraphs separated by blank lines..."
              ...
              {mergedSOP.details && (
                <div className="mb-4">
                  <h5 style={{ color: '#495057', fontWeight: '600', marginBottom: '1rem' }}>
                  ...
                  {mergedSOP.details.grammar && mergedSOP.details.grammar.length > 0 && (
                    <div className="mb-3">
                      <h6 style={{ color: '#dc3545', fontWeight: '600' }}>
                        <HiExclamationTriangle className="me-2" />
                        Grammar & Spelling Corrections:
                      </h6>
                      <div className="p-3 rounded" style={{ background: '#fff5f5', border: '1px solid #fed7d7' }}>
                              ...
                              Object.entries(item).map(([key, value]) => (
                                key !== 'kb_article_1' && key !== 'kb_article_2' && (
                                  <div key={key} style={{ fontSize: '0.9rem' }}>
                                    <span style={{ color: '#dc3545', fontWeight: '600' }}>"{key}"</span>
                                    <span className="mx-2">→</span>
                                    <span style={{ color: '#28a745', fontWeight: '600' }}>"{value}"</span>
                  ...
                  {mergedSOP.details.overlapping && mergedSOP.details.overlapping.length > 0 && (
                    <div className="mb-3">
                      <h6 style={{ color: '#0066cc', fontWeight: '600' }}>
                        <HiArrowsPointingIn className="me-2" />
                        Overlapping Content:
                      </h6>
                      <div className="p-3 rounded" style={{ background: '#f0f8ff', border: '1px solid #b3d9ff' }}>
                  ...
                  {mergedSOP.details.conflicting && mergedSOP.details.conflicting.length > 0 && (
                    <div className="mb-3">
                      ...
                      <div className="p-3 rounded" style={{ background: '#fffbf0', border: '1px solid #ffecb3' }}>
                  ...
                  {mergedSOP.details.complementary && mergedSOP.details.complementary.length > 0 && (
                    <div className="mb-3">
                      ...
                      <div className="p-3 rounded" style={{ background: '#f0fff4', border: '1px solid #c3e6cb' }}>
              ...
              <div className="mb-3">
                <h6 style={{ color: '#495057', fontWeight: '600' }}>
                  <HiDocumentDuplicate className="me-2" />
                  Original Sources:
                </h6>
                <Row>
        ...
        <Modal.Body style={{ maxHeight: '75vh', overflowY: 'auto' }}>
          {filteredMergedSOPs.length === 0 ? (
            <div className="text-center py-5">
              ...
              <h5 className="mt-3 text-muted">No Merged SOPs Yet</h5>
            ...
            <div>
              {/* Summary Stats */}
              <div className="mb-4 p-3 rounded" style={{ background: 'linear-gradient(135deg, #e8f5e8 0%, #f0f8f0 100%)', border: '1px solid #28a745' }}>
                <Row>
                  <Col md={3}>
                    <div className="text-center">
                      ...
                      <small className="text-muted">Total Merged SOPs</small>
                    </div>
                  </Col>
                  <Col md={3}>
                    <div className="text-center">
                      ...
                      <small className="text-muted">Sources Combined</small>
                    </div>
                  </Col>
                  <Col md={3}>
                    <div className="text-center">
                ...
                {filteredMergedSOPs.map((sop, index) => (
                  <Col lg={6} className="mb-4" key={sop.id}>
                    ...
                    >
                      ...
                      <div className="card-footer bg-light" style={{ borderRadius: '0 0 10px 10px' }}>
                        <div className="d-flex justify-content-between align-items-center">
      ...
      <Modal show={showLowScoringModal} onHide={() => setShowLowScoringModal(false)} size="xl" centered>
        <Modal.Header closeButton style={{ background: 'linear-gradient(135deg, #feca57 0%, #ff9ff3 100%)', color: 'white' }}>
          <Modal.Title>
              ...
              <div className="mb-4">
                ...
                {!isEditingRefinedArticle ? (
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: refinedArticle.htmlContent }}
                  />
                ) : (
                  <div>
                    <div className="mb-2 p-2" style={{ 
