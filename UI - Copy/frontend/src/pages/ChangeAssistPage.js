...
const ChangeAssistPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const requestBody = {
        ...
        uploaded_files: savedFileName ? savedFileName : "", // Use uploaded file name or leave blank
      ...
      const credentials = btoa(`${apiUserName}:${apiPass}`);
      ...
      const response = await axios.post(
        'http://172.31.6.97:6500/change_management/api/v1/draft_change_prompt/',
      ...
      console.log(apiData, response, "apiData iiiii")
      setApiResponse(apiData);

      // Parse change_details from the API response
      if (apiData.data.output && apiData.data.output.data && apiData.data.output.data.change_details) {
        const changeDetails = apiData.data.output.data.change_details;
        const impactAnalysis = apiData.data.output.data.impact_analysis;
    ...
    if (file) {
      ...
      if (file) {
        ...
        try {
          const apiUserName = 'rest'
          const apiPass = '!fi$5*4KlHDdRwdbup%ix'
          // const response = await fetch('http://172.31.6.97:6500/remote_file_management/api/v1/change_management_file/upload_image/', {
          //   method: 'POST',
          //   body: imageFormData,
          //   auth: {
          //     username: apiUserName,
          ...
          // const response = await axios.post(
          //   'http://172.31.6.97:6500/remote_file_management/api/v1/change_management_file/upload_image/',
          //   {...imageFormData},
          //   {
          //     headers: {
          //       'Content-Type': 'application/json'
          ...
          if (response.ok) {
            ...
            setSavedFileName(result.saved_as)
          } else {
  ...
  const handleSubmitChange = async () => {
    ...
    try {
      ...
      // Create request body
      const requestBody = {
        uploaded_files: `${savedFileName}`, // Static for now as requested
      ...
      const response = await axios.post(
        'http://172.31.6.97:6500/change_management/api/v1/create_change_with_impact/',
        requestBody,
        {
          headers: {
            'Content-Type': 'application/json'
                ...
                <form className="space-y-6 flex flex-col">
                  ...
                  <div className="flex justify-end space-x-4 pt-6 border-t border-white/10">
                    <button
                      ...
                      className="px-6 py-3 bg-gray-600/20 hover:bg-gray-600/30 text-white font-medium rounded-lg transition-colors duration-300"
                    >
                      Cancel
                    </button>
                    <button
          ...
          >
            ...
            <div className="flex space-x-3">
              <button
                ...
                className="flex-1 px-4 py-2 bg-gray-600/20 hover:bg-gray-600/30 text-white font-medium rounded-lg transition-colors duration-300"
              >
                Create Another
              </button>
              <button
