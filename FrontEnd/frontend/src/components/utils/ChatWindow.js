            ...
            <div key={pIndex} className="space-y-2">
              {lines.map((line, lIndex) => {
                ...
                if (numberedStepMatch) {
                  ...
                  return (
                    <div key={lIndex} className="text-white">
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-dark-800/30 border-t border-white/20">
      {chatHistory.length === 0 ? (
        ...
        chatHistory.map((message, index) => (
          ...
          >
            <div className={`items-start space-x-3 max-w-sm lg:max-w-lg ${message.sender === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
              {/* Avatar */}
              <div className={` h-8 rounded-full ${message.sender === 'user'
                ? 'text-white'
                : ' text-white'
                }`}>
                {message.sender === 'user' ? (
                  ...
                  <div className='flex'>
                    ...
                    <p className='p-2 text-sm font-bold'>Tech Assist</p>
                  </div>
                )}
              </div>
              {/* Message bubble */}
